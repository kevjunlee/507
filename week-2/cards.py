...
class TestCard(unittest.TestCase):
	...
	def test_card_suit(self):
		club_card = Card(suit = 1)
		self.assertEqual(club_card.suit, "Clubs")
	
	def test_king_spades(self):
		spades_king = Card(suit = 3, rank = 13)
		self.assertEqual(spades_king.suit, "Spades")
		self.assertEqual(spades_king.rank, "King")
	...
	def test_replace_card(self):
		...
		missing_card = []
		deck2.replace_card(missing_card)
	...
	def test_replace_correct(self):
		...
		deck3.pop_card()
		deck3.pop_card()
		self.assertEqual(len(deck3.cards), 50)
		missing_cards = []
		deck3.replace_card(missing_cards)
		deck3.replace_card(missing_cards)
		self.assertEqual(len(deck3.cards), 51)
	
	def test_tuple(self):
		played_war = play_war_game()
		print(str(played_war))
		if any(s in played_war[0] for s in played_war[0]):
			print("there is a string")
	
	def queen_of_hearts_test(self):
		queen_of_hearts = Card(suit = 2, rank = 12) #creates a card instance of rank queen and suit hearts
		self.assertEqual(queen_of_hearts.rank, "Queen") #tests to see if the rank returns "queen"
		self.assertEqual(queen_of_hearts.suit, "Hearts") #tests to see if suit returns "hearts"
